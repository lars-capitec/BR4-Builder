...
const primaryButtonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap font-bold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2F70EF] focus-visible:ring-offset-2 disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      size: {
        default: "h-12 px-6 gap-2 text-base leading-6 [&_svg]:size-6 rounded-full",
        small: "h-8 px-4 gap-1 text-sm leading-5 [&_svg]:size-4 rounded-full",
      },
      state: {
        enabled: "bg-[#2F70EF] text-white hover:bg-[#2F70EF] hover:before:absolute hover:before:inset-0 hover:before:bg-[rgba(0,51,160,0.12)] hover:before:rounded-full hover:before:content-[''] relative",
        disabled: "bg-[rgba(31,39,46,0.12)] text-[#1F272E] opacity-40",
      },
      iconPosition: {
        leading: "flex-row",
...
const PrimaryButton = React.forwardRef<HTMLButtonElement, PrimaryButtonProps>(
  ({ className, size, state, iconPosition, asChild = false, icon, children, disabled, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(primaryButtonVariants({ 
          size, 
          state: buttonState, 
