...
export default function LoyaltyCardPersonalization() {
  ...
  const [numberOfPurchases, setNumberOfPurchases] = useState(5);
...
•	Offer cannot be sold or otherwise bartered`);
  ...
  const handleBack = () => {
    navigate("/loyalty-card-templates");
  };
  ...
  // Stamp controls
  const handleIncreasePurchases = () => {
    if (numberOfPurchases < 10) {
      setNumberOfPurchases(numberOfPurchases + 1);
    }
  };

  const handleDecreasePurchases = () => {
  ...
  const stamps = Array.from({ length: numberOfPurchases }, (_, index) => ({
    id: index,
    completed: index === 0 // First stamp is completed as example
  }));
            ...
            <div className="px-4 pb-4">
              <div className="bg-white rounded-xl overflow-hidden shadow-lg">
                ...
                <div className="pt-12 pb-6 px-6 space-y-4">
                  ...
                  {/* Number of Purchases Control */}
                  <div className="flex flex-col items-center gap-4">
                    <div className="flex items-center gap-4">
                      <div className="space-y-2">
                        <Label className="text-sm font-bold text-[#4A535C]">No. of purchases</Label>
                        <div className="w-24 h-12 border border-[#A1ACBA] rounded-xl flex items-center justify-center">
                          <span className="text-base font-normal text-[#0F2132]">{numberOfPurchases}</span>
                        </div>
                      </div>

                      <div className="flex flex-col">
                        <button
                          onClick={handleIncreasePurchases}
                          className="p-2 hover:bg-gray-100 rounded-lg"
                        >
                          <ChevronUp className="w-6 h-6 text-[#009DE0]" />
                        </button>
                        <button
                          onClick={handleDecreasePurchases}
                          className="p-2 hover:bg-gray-100 rounded-lg"
                        >
                    ...
                    {/* Stamps Preview */}
                    <div className="flex justify-center gap-2 flex-wrap max-w-[280px]">
                      {stamps.map((stamp) => (
                        <div
                          key={stamp.id}
                          className={`w-12 h-12 rounded-full flex items-center justify-center ${
